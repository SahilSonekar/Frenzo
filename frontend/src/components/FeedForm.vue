<template>
  <form @submit.prevent="submitForm" method="post">
    <div class="p-4">
      ...
      <div v-if="errorMessage" class="p-3 mt-2 rounded-lg bg-red-100 text-red-700 border border-red-300 dark:bg-red-900 dark:text-red-200 dark:border-red-700">
        {{ errorMessage }}
      </div>
  ...
  methods: {
    submitForm() {
      ...
      axios
        ...
        .catch((error) => {
          console.log("CATCH TRIGGERED", error.response);
          if (error.response && error.response.data && error.response.data.error) {
            this.errorMessage = error.response.data.error;
          } else {
            this.errorMessage = "Something went wrong. Please try again.";
          }
          console.log("errorMessage is now:", this.errorMessage);
        });
    },
    toggleDark() {
